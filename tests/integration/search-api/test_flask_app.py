"""
Tests for search-api Flask application behavior.

Run all Flask app tests:
    TEST_API="search-api" pytest tests/localhost/integration/search-api/test_flask_app.py -v
"""
...
def test_indices_returns_list_or_dict(base_url):
    """Test that /indices returns proper data structure."""
...
def test_flask_handles_large_payloads(base_url):
    ...
    r = requests.post(
        f"{base_url}/search",
        json={"query": {"match_all": {}}, "size": 10000, "from": 0, "extra": "x" * 100000},
        headers={"Content-Type": "application/json"},
        timeout=TIMEOUT
    )
